# https://www.acmicpc.net/problem/7568 (상호비교 리스트안에 리스트, 숫자로변환)
...
a = []
for i in range(n):
    # a.append(input().split()) #오답(why) => 출력은 맞음 
    a.append(list(map(int,input().split()))) #정답 => 정규 방식 
...
이렇게 등수를 결정하면 같은 덩치 등수를 가진 사람은 여러 명도 가능하다. 아래는 5명으로 이루어진 집단에서 
...
출력
여러분은 입력에 나열된 사람의 덩치 등수를 구해서 그 순서대로 첫 줄에 출력해야 한다. 단, 
각 덩치 등수는 공백문자로 분리되어야 한다.
'''