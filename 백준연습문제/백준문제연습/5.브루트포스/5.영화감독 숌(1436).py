# https://www.acmicpc.net/problem/1436 (무한루프조건)


n = int(input())
d = 666
while n > 0: #0이 될때까지 무한 루프
    if '666' in str(d):
        n-=1 #조건이 맞으면 1차감 (처음 666은 조건 맞아서 곧바로 차감 n=0 탈출)
        if n == 0:
            res = d
    d+=1 #d 1씩 증가
...
두 번째 영화의 제목은 "세상의 종말 1666"와 같이 이름을 지을 것이다. 일반화해서 생각하면, N번째 영화의 제목은 세상의 종말 (N번째로 작은 종말의 수) 와 같다.
