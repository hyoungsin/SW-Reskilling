...
for i in range(k):
    apple = int(input())  #사과 위치
    
    if left <= apple and apple <= right:  #바구니 안에 떨어지면
        continue
        
    if left > apple:  #바구니 왼쪽에 가깝게 떨어지면
        cnt += (left - apple)
        right -= (left - apple)
        left = apple
...
입력
첫째 줄에 N과 M이 주어진다. (1 ≤ M < N ≤ 10) 둘째 줄에 떨어지는 사과의 개수 J가 주어진다. (1 ≤ J ≤ 20) 다음 J개 줄에는 사과가 떨어지는 
위치가 순서대로 주어진다.
