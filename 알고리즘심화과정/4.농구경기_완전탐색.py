# https://www.acmicpc.net/problem/1159 (set, count or (count sorting))
...
def main():
    ...
    for i in range(26):
        if counting[i] >= 5:
            flag = True # 5개 이상일경우 flag 속성을 기록함 
            # 아스키 코드를 문자로 바꿔서 출력
...
어느 날 그에게 지방의 한 프로농구팀을 감독할 기회가 생기게 되었다. 그는 엄청난 지도력을 보여주며 프로 리그에서 우승을 했고, 
이제 국가대표팀의 감독이 되었다.
