...
def check(mid):
    global N,M,a,res # m : 절단높이 
    sum = 0 
    for i in a: 
        if i-mid < 0 : # 음수면 버림 (나무높이에서 차감하면 안됨) 
            continue
        sum += i-mid # 그외는 sum에 추가시킴 
    return sum >= M # m보다 큰경우에만 true반환 
...
def main():
    ...
    lo = 0 # 이진탐색을 위해서 하단과 상단 정의함 (0 ~ 제일높은나무)
    hi = max(a) 

    while lo <= hi: # 최소값이 최댓값과 일치될때까지 반복 
        mid = (lo+hi) // 2 # 중간값 찾기 
        if check(mid): # 찾은 중간값이 sum조건 만족하면 길이 확보 min값만 높이면 됨 (늘리기만 하면됨 그러면 나무길이가 줌)
            lo = mid+1 # min값을 1씩 증가 (ma와 같아질때까지)
            res = mid # 만족하는 숫자의 최종 mid 
        else:
            hi = mid-1 # sum이 만족스럽지 못할 경우 높이를 낮춤 
